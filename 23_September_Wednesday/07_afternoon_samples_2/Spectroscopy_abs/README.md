# Day 3 — Wednesday 23 September (Afternoon): Samples - X-ray Absorption

*PaNRAID School, Day 3 afternoon — follows the `McStas_McXtrace_Sample_Components` lecture*

For this exercise, we shall build a simplified model of the ROCK@SOLEIL absorption beam-line.
...
To select an energy we need to insert a channel cut double crystal monochromator. There are different ways of doing this. In this example we will use two `Bragg_crystal` components to materialise the channel cut. You may as well refer to the `Template_DCM` example.
