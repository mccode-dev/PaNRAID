...
| File | Corresponds to |
|------|-----------------|
| `ex_powderN_mcstas_starter.instr` | Part A, Exercise A1 (McStas): source + `PowderN(reflections="Al.laz")` + beamstop + `PSD_monitor` |
| `ex_powderN_mcxtrace_starter.instr` | Part B, Exercise B1 (McXtrace): source + `PowderN(reflections="LaB6.cif")` + beamstop + `PSD_monitor` |
...
run. Exercise A2/B2 (comparing the same material described a second way —
...
The A3/B3 "attach yesterday's optics" option likewise isn't covered by a
starter file — it's a matter of replacing the source in these same files
